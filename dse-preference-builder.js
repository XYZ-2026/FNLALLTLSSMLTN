...
let collegeMetadata = [];
let scoreType = 'pct'; // Default: 'pct' (Diploma Percentile)
...
const CATS = {
  ...
  'Biotech & Allied': ['BIOTECHNOLOGY', 'BIO-MEDIA', 'BIO MEDICAL', 'FOOD', 'AGRICULTURE', 'PHARMACEUTICAL'],
  'Other Branches': []
};
...
function switchScoreType(type) {
  scoreType = type;
  document.getElementById('pctGroup').style.display = type === 'pct' ? 'block' : 'none';
  document.getElementById('rankGroup').style.display = type === 'rank' ? 'block' : 'none';

  const tabPct = document.getElementById('tabPct');
  const tabRank = document.getElementById('tabRank');

  if (type === 'pct') {
    tabPct.style.background = 'var(--brand)';
    tabPct.style.color = '#fff';
    tabRank.style.background = 'transparent';
    tabRank.style.color = 'var(--muted)';
  } else {
    tabRank.style.background = 'var(--brand)';
    tabRank.style.color = '#fff';
    tabPct.style.background = 'transparent';
    tabPct.style.color = 'var(--muted)';
  }
}
...
function startNewForm() {
  ...
  document.getElementById('inGender').value = 'G';
  document.getElementById('inRegion').value = 'all';
...
function loadForm(formId, step = 1) {
  ...
  document.getElementById('inGender').value = form.gender || 'G';
  document.getElementById('inRegion').value = form.region || 'all';
  document.getElementById('inMinority').value = form.minority || '';

  if (form.rank) {
    document.getElementById('inRank').value = form.rank;
    document.getElementById('inPct').value = form.percentile || '';
    switchScoreType('rank');
  } else {
    document.getElementById('inPct').value = form.percentile || '';
    document.getElementById('inRank').value = '';
    switchScoreType('pct');
...
async function deleteForm(formId) {
  ...
  const res = await authApi('deleteDsePrefForm', { userId: currentUserId, formId });
  if (res.ok) {
...
async function saveNonLockedData() {
  ...
  const res = await authApi('saveDsePrefData', {
...
function validateStep1() {
  const p = document.getElementById('inPct').value.trim();
  const r = document.getElementById('inRank').value.trim();

  if (scoreType === 'pct' && (!p || isNaN(parseFloat(p)))) {
    pbToast('Please enter a valid Diploma percentile');
    return false;
  }
  if (scoreType === 'rank' && (!r || isNaN(parseInt(r)))) {
    pbToast('Please enter your State Merit Rank');
    return false;
  }
  return true;
}
...
/* ══════ DSE COLLEGE MATCHING (Step 3) ══════ */
function generateMatches() {
  const userPct = parseFloat(document.getElementById('inPct').value) || 0;
  const userRank = parseInt(document.getElementById('inRank').value) || 999999;
  const cat = document.getElementById('inCategory').value;
  const gender = document.getElementById('inGender').value;
  const region = document.getElementById('inRegion').value;
  const minorityPref = document.getElementById('inMinority').value;
  ...
  let filtered = allData.filter(r => {
    ...
    if (cat === 'EWS') {
      if (r.seatType !== 'EWS') return false;
    } else {
      const st = r.seatType || '';
      if (!st.startsWith(gender) || !st.includes(cat)) return false;
    }

    // 2. Region filter
    if (region !== 'all') {
      if (!String(r.code || '').startsWith(region)) return false;
    }
    ...
    if (minorityPref) {
      ...
      if (!status.toLowerCase().includes(minorityPref.toLowerCase())) return false;
    }
  ...
  // Group by institute code+branch, keep the one with best (highest percentile / lowest rank)
  const groups = {};
  filtered.forEach(r => {
    const key = r.code + '|' + r.branch;
    if (scoreType === 'pct') {
      if (!groups[key] || r.percentile > groups[key].percentile) groups[key] = r;
    } else {
  ...
  results = results.map(r => {
    ...
    const isGov = status.includes('government');
    const isAided = status.includes('aided') && !status.includes('un-aided');
    const isAuto = status.includes('autonomous');
    const isMinority = status.includes('minority');
    const minorityType = extractMinority(meta.status || '');

    // Aspirational flag (score is better/higher than user's)
    let isAsp = false;
    if (scoreType === 'pct') {
      isAsp = r.percentile > userPct;
    } else {
      isAsp = r.rank < userRank;
    }
    ...
    return {
      ...
      status: meta.status || '',
      isGov, isAided, isAuto, isMinority, minorityType,
      isAspirational: isAsp
    };
  });

  // Sort: Reachable (user can get it) vs Aspirational
  const reachable = results.filter(r => !r.isAspirational).sort((a, b) => b.percentile - a.percentile).slice(0, 40);
  const aspirational = results.filter(r => r.isAspirational).sort((a, b) => a.percentile - b.percentile).slice(0, 6);

  matchedColleges = [...aspirational, ...reachable];

  // Build suggestion pool
  suggestionPool = results.filter(r => {
    return !matchedColleges.some(m => m.code === r.code && m.branch === r.branch);
  ...
  grid.innerHTML = displayItems.map((c, idx) => {
    ...
    if (isLocked) {
      return `<div class="col-card locked" style="cursor: default; opacity: 0.7; pointer-events: none; position: relative;">
        ...
        <div class="col-pct" style="filter: blur(3.5px); user-select: none;"><strong>••.•%</strong> <small>Rank: ••••</small></div>
      </div>`;
    }
    ...
    return `<div class="col-card ${sel ? 'selected' : ''} ${asp}" onclick="toggleCollege(${realIdx})">
      ...
      <div class="col-pct"><strong>${c.percentile.toFixed(2)}%</strong> <small>Rank: ${c.rank.toLocaleString()}</small></div>
    </div>`;
  }).join('');
...
function buildPrefList() {
  ...
  // Sort user selected by percentile desc
...
function renderPrefList() {
  ...
  list.innerHTML = displayPref.map((c, i) => {
    ...
    if (isLocked) {
      return `
        <tr class="pref-item locked-row" draggable="false" data-idx="${i}" style="cursor: default;">
          ...
          <td class="excel-td-pct" style="vertical-align: middle"><strong style="filter: blur(2.5px); opacity: 0.5;">••.••%</strong> <br/> <small style="color:var(--muted); filter: blur(2px)">Rank: ••••</small></td>
    ...
    const statusTags = [];
    if (c.isGov) statusTags.push('<span class="col-tag gov" style="font-size: 9px; padding: 2px 8px">Government</span>');
    if (c.isAuto) statusTags.push('<span class="col-tag auto" style="font-size: 9px; padding: 2px 8px">Autonomous</span>');
    if (c.isAided) statusTags.push('<span class="col-tag" style="font-size: 9px; padding: 2px 8px">Aided</span>');
    if (c.isMinority) statusTags.push(`<span class="col-tag minority" style="font-size: 9px; padding: 2px 8px">${escH(c.minorityType || 'Minority')}</span>`);
    ...
    return `
      <tr class="pref-item ${isFixed ? 'is-fixed' : ''} ${c.isAspirational ? 'asp-item' : ''}" 
        ...
        <td class="excel-td-pct" style="vertical-align: middle"><strong>${c.percentile ? c.percentile.toFixed(2) + '%' : 'N/A'}</strong> <br/> <small style="color:var(--muted)">Rank: ${c.rank ? c.rank.toLocaleString() : 'N/A'}</small></td>
...
function renderAspirational() {
  ...
  if (suggGrid) {
    ...
    const pool = suggestionPool.filter(c => !inPref.has(c.code + '|' + c.branch) && c.percentile > userPct).slice(0, 8);
...
function searchManualColleges() {
  ...
  searchTimeout = setTimeout(async () => {
    ...
    let results = Object.values(groups).map(r => {
      ...
      const status = (meta.status || '').toLowerCase();
      return {
        ...r, instituteName: r.name, status: meta.status || '',
    ...
    resDiv.innerHTML = sliced.map(r => {
      ...
      const tags = [];
      if (r.isGov) tags.push('<span class="col-tag gov">Government</span>');
      if (r.isAuto) tags.push('<span class="col-tag auto">Autonomous</span>');
      ...
      return `<div class="col-card ${isSel ? 'selected' : ''}" style="margin-bottom:12px; cursor: default">
        ...
        <div class="col-pct"><strong>${r.percentile.toFixed(2)}%</strong> <small>Code: ${r.code}</small></div>
...
function toggleAspirational(code, branch) {
  ...
  if (idx >= 0) {
    ...
    if (!c) {
      const raw = allData.find(r => r.code === code && r.branch === branch);
      if (raw) {
        const metaMap = {}; collegeMetadata.forEach(m => metaMap[m.code] = m);
        const meta = metaMap[code] || {};
        const status = (meta.status || '').toLowerCase();
        c = {
          ...raw, instituteName: raw.name, status: meta.status || '',
          isGov: status.includes('government'), isAuto: status.includes('autonomous'),
          isMinority: status.includes('minority'), minorityType: extractMinority(meta.status || ''),
          isAided: status.includes('aided')
        };
...
function handleAddSuggestion(code, branch, isAspirational = false) {
  ...
  if (c) {
    const idx = prefList.findIndex(p => p.code === c.code && p.branch === c.branch);
    if (idx < 0) {
      const copy = { ...c };
      if (isAspirational) {
...
/* ══════ DSE FIRESTORE INTEGRATION ══════ */
async function loadSavedPrefData() {
  if (!currentUserId) return;
  const res = await authApi('getDsePrefData', { userId: currentUserId });
  if (res.ok && res.data) {
    prefDataLoaded = true;
    ...
    if (allForms.length > 0) {
      ...
      document.getElementById('inCategory').value = latest.category || 'OPEN';
      document.getElementById('inGender').value = latest.gender || 'G';
      document.getElementById('inRegion').value = latest.region || 'all';
      document.getElementById('inMinority').value = latest.minority || '';
        ...
        dashList.innerHTML = allForms.map(form => {
          ...
          return `<div class="col-card dashboard-form-card" style="text-align: left; border: 1px solid var(--stroke); padding: 20px; cursor: default; margin-bottom: 12px">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px; flex-wrap: wrap; gap: 8px">
              <div>
                <div style="font-weight: 800; color: var(--brand); font-size: 17px">DSE Preference List</div>
            ...
            <div class="dash-card-stats" style="margin-bottom: 20px; padding: 16px; background: #f8fafc; border-radius: 16px; border: 1px solid var(--stroke); display: flex; flex-direction: column; gap: 12px">
              <div style="display: flex; justify-content: space-between; align-items: center; font-size: 13px">
                <span style="font-weight: 800; color: var(--ink)">Score: ${form.percentile ? form.percentile.toFixed(2) + '%' : (form.rank || '??')}</span>
                <span style="font-weight: 700; color: var(--brand); font-size: 11px">Region: ${form.region === 'all' ? 'All' : 'Region ' + form.region}</span>
              </div>
              <div style="font-size: 12px; font-weight: 600; color: var(--ink2); padding-top: 8px; border-top: 1px dashed var(--stroke); display: flex; justify-content: space-between">
                ...
                <span style="color:var(--muted)">${form.gender === 'L' ? 'Ladies' : 'General'}</span>
              </div>
            </div>
...
function unlockProfileFields() {
  ...
  const elR = document.getElementById('inRank');
  if (elR) { elR.disabled = false; elR.style.opacity = '1'; }
}
...
async function submitEditRequest() {
  const user = getSession();
  if (!user) return;
  const msg = (document.getElementById('reportMessage').value || '').trim() || 'Please unlock my DSE preference list edits.';
  const btn = document.getElementById('submitReportBtn');
  if (btn) { btn.disabled = true; btn.textContent = 'Sending...'; }
  const res = await authApi('submitEditRequest', { userId: currentUserId, userName: user.name, userEmail: user.email, message: msg, tool: 'dse-pref-builder' });
...
function exportPDF() {
  ...
  doc.setFontSize(14);
  doc.text('DSE Option Form Preference Report', 14, 28);

  doc.setFont("Helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(107, 114, 128);
  doc.text(`Percentile: ${pct}% | Rank: ${rank || 'N/A'} | Category: ${cat} | Gender Pool: ${gender === 'L' ? 'Ladies' : 'General'}`, 14, 34);
  ...
  doc.save(`DSE_Preferences_${rank || 'List'}.pdf`);
...
function exportExcel() {
  ...
  const wsData = [
    ['Choice Number', 'Institute Code', 'Institute Name', 'Branch Name', 'Closing Cutoff %', 'State Merit Rank']
  ];
  ...
  XLSX.utils.book_append_sheet(wb, ws, "DSE Preferences");
  XLSX.writeFile(wb, `DSE_Preferences_${Date.now()}.xlsx`);
...
function extractMinority(status) {
  const match = (status || '').match(/minority\s*-\s*([^)]+)/i);
...
window.switchScoreType = switchScoreType;
